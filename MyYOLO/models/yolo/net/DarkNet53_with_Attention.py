...
from models.yolo.net.config import configDarkNet53 as cfg
...
class DarkNet53:
    ...
    def _feature_extractor_layer(self, inputs, training):
        with tf.variable_scope('visualization'), tf.device('/CPU:0'):
        ...
        with tf.variable_scope('darknet_53'):
            with tf.device('/gpu:0'):
                ...
                # attention2
                self.attention2 = self._Conv2d(layer, filters=1, shape=[1, 1], stride=(1, 1), alpha=self.alpha,
                                               training=training, name='_Attention2')
                ...
                # attention1
                self.attention1 = self._Conv2d(layer, filters=1, shape=[1, 1], stride=(1, 1), alpha=self.alpha,
                                               training=training, name='_Attention1')
                ...
                # attention0
                self.attention0 = self._Conv2d(layer, filters=1, shape=[1, 1], stride=(1, 1), alpha=self.alpha,
                                               training=training, name='_Attention0')
        ...
        with tf.name_scope('detection_layer'):
            with tf.variable_scope('scale_0'):
                with tf.device('/gpu:0'):
                    ...
                    self.scale_0 = tf.multiply(self.scale_0, self.attention0)
            ...
            with tf.variable_scope('scale_1'):
                with tf.device('/gpu:0'):
                    ...
                    self.scale_1 = tf.multiply(self.scale_1, self.attention1)
            ...
            with tf.variable_scope('scale_2'):
                with tf.device('/gpu:0'):
                    ...
                    self.scale_2 = tf.multiply(self.scale_2, self.attention2)
    ...
    def get_output(self):
        return [self.scale_0, self.scale_1, self.scale_2], [self.attention0, self.attention1, self.attention2]
