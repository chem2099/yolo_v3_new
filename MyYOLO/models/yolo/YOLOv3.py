...
class YOLOv3(object):
    ...
    def calculate_no_object_confidence_loss(self, no_object_confidence_hat, no_object_confidence, no_object_mask, ignore_mask, scope='no_object_confidence_loss'):
        with tf.name_scope(scope):
            no_object_loss = tf.reduce_sum(no_object_mask * tf.square(no_object_confidence - no_object_confidence_hat) * ignore_mask) / self.batch_size
    ...
    def total_loss(self, labels, scope='total_loss'):
        with tf.name_scope(scope):
            ...
            for i in range(len(self.scales)):
                ...
                ignore_mask = tf.cast(iou < self.ignore_thresh, tf.float32) # (None, 13, 13, 5)
                ignore_mask = tf.expand_dims(ignore_mask, axis=4) # (None, 13, 13, 5, 1)
