...
def zmMinFilterGray(src, r=7):
    ...
    h, w = src.shape[:2]
    I = src
    res = np.minimum(I, I[[0] + [i for i in range(h - 1)], :])
    res = np.minimum(res, I[[i for i in range(1, h)] + [h - 1], :])
    I = res
    res = np.minimum(I, I[:, [0] + [i for i in range(w - 1)]])
    res = np.minimum(res, I[:, [i for i in range(1, w)] + [w - 1]])
...
def deHaze(m, r=56, eps=0.001, w=0.80, maxV1=0.6, bGamma=False):
    Y = np.zeros(m.shape)
